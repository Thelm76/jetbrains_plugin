...
- **Never set project as the parent disposable** - Instead, always use `SweetProjectService.getInstance(project)` as the parent disposable
- This ensures proper lifecycle management and prevents memory leaks
- Example:
  ```kotlin
  // ✅ Correct - Using SweetProjectService as parent disposable
  project.messageBus.connect(SweetProjectService.getInstance(project)).subscribe(
...
### Icon Colorization
- **Use `colorizeIcon(icon, color)`** from `dev.sweet.assistant.utils` instead of the deprecated `IconUtil.colorize`
...
- Example:
  ...
  import dev.sweet.assistant.utils.colorizeIcon
...
- Example file structure:
  ...
  ```
- In SweetIcons.kt, simply load the base icon:
  ```kotlin
