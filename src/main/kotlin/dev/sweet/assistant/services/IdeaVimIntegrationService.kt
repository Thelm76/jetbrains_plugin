...
class IdeaVimIntegrationService(
    ...
    companion object {
        ...
        private const val SWEET_TAB_MAPPING = "map <Tab> :action dev.sweet.assistant.autocomplete.edit.AcceptEditCompletionAction<CR>"
        private const val SWEET_MAPPING_COMMENT = "\" Sweet AI Tab completion mapping"
    ...
    /**
     * Checks if the Sweet Tab mapping already exists in .ideavimrc
     */
    private fun hasSweetTabMapping(ideavimrcFile: File): Boolean {
        ...
        return try {
            ...
            content.contains(SWEET_TAB_MAPPING)
        } catch (e: Exception) {
    ...
    /**
     * Adds the Sweet Tab mapping to the .ideavimrc file
     */
    private fun addSweetTabMapping(ideavimrcFile: File) {
        try {
            val mappingWithComment = "\n$SWEET_MAPPING_COMMENT\n$SWEET_TAB_MAPPING\n"
            ...
            logger.info("Successfully added Sweet Tab mapping to .ideavimrc")
        } catch (e: Exception) {
            logger.warn("Error adding Sweet Tab mapping to .ideavimrc", e)
        }
    }
        ...
        ApplicationManager.getApplication().invokeLater {
            val notification =
                NotificationGroupManager
                    .getInstance()
                    .getNotificationGroup("Sweet AI Notifications")
                    .createNotification(
                        "IdeaVim Integration Complete",
                        "Sweet has configured your Vim settings for Tab completion. Please restart your IDE to activate the changes.",
    ...
    fun configureIdeaVimIntegration() {
        ApplicationManager.getApplication().executeOnPooledThread {
            ...
            if (hasSweetTabMapping(ideavimrcFile)) {
                logger.info("Sweet Tab mapping already exists in .ideavimrc")
                return@executeOnPooledThread
            }

            logger.info("IdeaVim detected, adding Sweet Tab mapping to .ideavimrc")
            addSweetTabMapping(ideavimrcFile)
