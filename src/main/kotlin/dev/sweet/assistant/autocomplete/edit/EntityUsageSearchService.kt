...
//        val endLine = startLine + maxOf(0, lines.size - 1)
    ...
    fun getCurrentLineEntityUsages(currentEditorState: EditorState): List<FileChunk> {
        ...
        try {
            ...
            val language = SweetConstants.EXTENSION_TO_LANGUAGE[currentFileExtension]
            val relevantKeywords = language?.let { SweetConstants.LANGUAGE_KEYWORDS[it] } ?: emptyList()

            val candidateTerms =
                searchText
                    .replace(Regex(SweetConstants.COMMON_SYMBOLS_REGEX), " ") // Remove common symbols
                    .split("\\s+".toRegex())
                    .filter { term ->
                        term.length >= 3 &&
    ...
    private fun isLanguageKeyword(
        ...
        val language = SweetConstants.EXTENSION_TO_LANGUAGE[fileExtension] ?: return false

        // Get the keywords for this language
        val keywords = SweetConstants.LANGUAGE_KEYWORDS[language] ?: return false
