...
import dev.sweet.assistant.autocomplete.Debouncer
...
private fun invokeLaterIfGatewayModeClient(
    ...
    if (SweetConstants.GATEWAY_MODE == SweetConstants.GatewayMode.CLIENT) {
...
class RecentEditsTracker(
    ...
    private val debouncer =
        Debouncer({ SweetConfig.getInstance(project).getDebounceThresholdMs() }, scope, project) { processLatestEdit() }
...
//                    }
    ...
    private fun shouldExcludeFromAutocomplete(filePath: String): Boolean {
        val exclusionPatterns = SweetConfig.getInstance(project).getAutocompleteExclusionPatterns()
    ...
    private fun processLatestEdit(triggeredManually: Boolean = false) {
        if (!triggeredManually && !SweetSettings.getInstance().automaticAutocompleteOn) {
            return
        }
            ...
            scope.launch {
                while (isActive) {
                    try {
                        ...
                        if (!request.triggeredManually && !SweetSettings.getInstance().automaticAutocompleteOn) {
                            continue
                        }
//                        println("Fetch jobs size: ${fetchJobs.size}")
        ...
        try {
            ...
            val request =
                NextEditAutocompleteRequest(
                    ...
                    privacy_mode_enabled = SweetConfig.getInstance(project).isPrivacyModeEnabled(),
                    client_ip = clientIp,
            ...
            NotificationDeduplicationService.getInstance(project).showNotificationWithDeduplicationAndErrorReporting(
                ...
                notificationGroup = "Sweet Autocomplete",
    ...
    fun queueAndTryToShowImportFixSuggestion(
        ...
        ApplicationManager.getApplication().invokeLater {
            ...
            if (current == null && !SweetSettings.getInstance().automaticAutocompleteOn) {
                suggestion.dispose()
                return@invokeLater
            }
